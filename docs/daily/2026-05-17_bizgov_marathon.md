...
## 🟢 사후 발견 2 — 시대적 정합성 (5/17 진짜진짜 EOD)

- 이재명 대통령 5/15 X "창업중심 국가" 발언
- 정부 정책 5대 키워드와 BizGovPlanner 정확 매칭 (창업중심 / 아이디어 / AI / 비수도권 / 사회문제)
- 모두의 창업 1차 합격자 130명 분포 부합 (39세 이하 63.8%, 비수도권 72.3%, AI 33.8%)
- 응모서 도입부 카피 영구 저장 — `docs/proposal/2026-07-03_jbtp_intro_copy.md`
- "모두의 창업" 2차 신청 가능성 추가 (6~7월 가능성, JBTP 7/3 와 병행)

→ **7/3 응모서의 시대적 정당성 확보**.

### 5/17 마라톤의 최종 의미 (5계층 가치 확보)

1. **시스템 진화** (완료) — 17,200 → 4,510, AI coverage 100%
2. **비전 명확화** (완료) — Phase 3 / Phase 4 단계 분할
3. **워크플로우 자산화** (완료) — 시뮬 누적 시스템 + 가설 정정 5건
4. **차별점 확보** (완료) — "단순 시스템 구축 ≠ 누적 학습 워크플로우"
5. **시대적 정합성** (NEW) — 정부 5대 키워드 정확 매칭, 응모서 핵심 카피

---

## 🟢 사후 발견 3 — 데이터 출처 신뢰성 다층 노출 (5/17 23:XX)

- 공식 API 1 (정부24 기업마당) + 자체 크롤러 7 (K-Startup, JBTP, JBBI, 전북경제통상진흥원, aT, 한국수산회, 전북외테크노파크) 명세 정리
- 다층 노출 전략 영구 저장 — `docs/backlog/data_source_trust_display.md`
- 5/18 작업 묶음에 추가 (1차 배너 20분 + 2차 메타 20분 = 40분)
- 응모서 도입부 카피와 시너지 — 정당성 + 노동 가치 + 신뢰성 3중 어필
- 3차 /about 페이지는 W21+ (응모서 후)

## 🟢 사후 발견 4 — 기관 공식 명칭 정정 (5/17 23:XX)

| source | 기존 | 공식 정정 |
|---|---|---|
| jbexport | 전북수출 | 전북경제통상진흥원 |
| at_global | AT | aT (공식 약자 소문자) |
| jbtp_related | JBTP 유관기관 | 전북외테크노파크 |

- 백로그: `docs/backlog/source_org_name_mapping.md`
- 응모서 + 신뢰성 표시 정확도 ↑

→ 응모서 5가지 차별점 통합 + 정확한 기관명 = 최종 완성

---

## 🟢 사후 발견 5 — 메일 시스템 검증 + 본인 사용자 신호 (5/17 EOD, 정정)

### 정정 사항 (이전 해석 오류)

이전 내 해석 (틀림):
- ❌ hover9710@gmail.com 으로 발송
- ❌ 사용자가 정부24, aT, 모두의 창업 정책 직접 메일 수신자

정확한 사실:
- ✅ 수신: hover1234@nate.com (사용자 본인 nate 메일)
  - 웹에서 회사 정보 테스트용으로 등록
  - BizGovPlanner 일반 사용자로서 매일 09:13 추천 메일 수신
- ✅ 별도 모니터링: hover9710@gmail.com
  - 크롤러 작동 안 할 시 오류 알림 받음 (사용자 기억, 5/18 확인 필요)
  - 운영 모니터링 목적
- ✅ 발송 시스템: Google Apps Script `sendDailyBizGovEmails`, 09:13 KST 매일 자동

### 응모서 6번째 차별점 — "scratched my own itch" 의 정확한 표현

- 개발자 본인이 BizGovPlanner 첫 사용자
- 회사 정보 등록 + daily 메일 수신
- 별도 운영 모니터링 메일 보유
- 운영자 = 사용자 = 검증자

→ 5/18 작업에 메일 시스템 검증 추가 — `docs/backlog/email_system_verification.md`
→ 응모서 6가지 차별점 통합 (정정 반영)

---

## 🟢 사후 발견 6 — 99.95% 정확화 (5/17 EOD)
...
| 전체 | 4,510 |
| 확인 필요 | 2,126 (47.1%) |
| 확인 필요 중 **bizinfo** | **2,125 (99.95%)** |
| 확인 필요 중 kstartup | 1 (0.05% outlier) |
| Render sync | 4,510 / 4,510 (pending=0) |

응모서 카피 갱신:

- 이전: "99.87%" / "47%" / 단편 표현
- 갱신: **"≈100% = 기업마당 단일 문제"** (실측 99.95%)
- 추가 메시지: "다른 7개 자체 크롤러 = status 처리 완벽"
- bizinfo dominance 강조 → Phase 3 작업 단위 4~6h 결정 재확인

운영 vs PC 차이 (확인 필요):

- 사용자 캡처 (오늘 운영): 전체 4,617 / 접수중 335
- PC DB: 전체 4,510 / 접수중 71
- 차이: +107 (전체), +264 (접수중)
- 원인 가설: 5/18 06:00 cron 이 운영 Render 에 직접 적재 (Actions → Render)
- 본질 변화 X — bizinfo dominance 는 cron 으로 강해지지 약해지지 않음
