...
> **예상 시간**: 30분~1h
> **분류**: product 정확도 개선
> **상태**: ✅ **완료 (2026-05-21, commit `44c20b0`)**

---

## ✅ 완료 (2026-05-21)

- `normalize_project.py` `infer_status` `open_keywords` 정정:
  - 버그 수정: `"예산 소진 시까지"`(공백) → `"예산 소진"`(부분일치) — 실데이터 `"예산 소진시까지"`(붙임)와 불일치하던 문제 해결
  - `"선착순"` 추가
- 신규 status 값('상시') 도입 X — 기존 `'접수중'`으로 흡수 (UI 필터·위젯 영향 0)
- "세부사업별 상이"/"차수별 상이"/"모집 완료시"는 보수적으로 제외 — `'확인 필요'` 유지 (infer_status 원칙 "좁게 시작")
- 결과: bizinfo 확인 필요 **1,446 → 905 (−541)** — 추정 ~600에 근접

## 5/21 추가 — 키워드 확장 + 한계 발견 (commit `7de6512`)

- `open_keywords` += 수시, 연중
- ⚠️ **즉시 효과 ~0** — DB 실측: 잔여 확인필요 905건 중 **period_text 빈값 833건 (92%)**. 키워드별 분포: 수시 1 / 연중 0 / 추후 9 / 상이 50 → 키워드 추가로 줄일 행이 거의 없음.
- → 무마감 키워드 분류는 사실상 한계 도달. 확인필요 추가 감소의 본질은 **period_text 부재 833건** 진단 → 신규 백로그 `docs/backlog/unknown_empty_period_text.md`.
