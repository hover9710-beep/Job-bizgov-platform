...
→ 우회: `sync_jbtp_v1_to_render.py` (v2 백필 ground-truth → 운영 DB UPSERT, HTTP 호출 없음).

⚠️ **2026-05-10 운영 stale 추가 발견**: 운영 DB jbtp 113건 (정지 상태, proxy 미작동).
v2 snapshot 128건 — 차이 15 = 5/4~5/7 새로 등록된 사이트 row. UPDATE only sync 로
는 누락 → **UPSERT 확장** (snapshot_only url INSERT) + **full row dump** 필요.
...
#### sync 스크립트 정책 (UPSERT 확장)
- **데이터 출처**: v2 로컬 DB (053 백필 apply 완료) → `v1_jbtp_dump.json` snapshot (128 row, full schema)
...
- **INSERT 경로** (snapshot url ∉ 운영): full row (052 sync_two_rows 패턴, INSERTABLE_COLS 30개)
  - 안전 가드: title 누락 시 skip
- **멱등성**: 같은 값이면 UPDATE 0. 두 번 실행해도 안전.
- **사이트 영향**: 0 (HTTP 호출 없음)

#### v1 로컬 시뮬 (운영 113건 stale 상태 재현)
v1 DB 사본 → 최신 15 url (notice_order DESC) 삭제 → 113건 stale 시뮬:
```
matched: 113  updated: 107  unchanged: 6
inserted: 15  snapshot_only_skip: 0  no_match: 0
```
- BEFORE (sim): total=113, notice_chk=1=0, start_date 채워짐=26, notice_order>0=0
- AFTER (sim):  total=128, notice_chk=1=32, start_date 채워짐=122, notice_order>0=122
