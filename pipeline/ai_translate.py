...
_BATCH_SYSTEM_PROMPT = (
    "여러 정부 지원사업 공고를 사용자 친화 언어로 통역한다. 규칙:\n"
    "1. friendly_title: 핵심만 15자 이내. 행정 기관 공식 명칭 핵심이면 유지.\n"
    "2. friendly_summary: 누가/뭘/언제까지 한 줄 30자 이내. 본문에 없는 내용 추측 금지.\n"
    "3. 부정 표현 (사기/의심/위험 등) 절대 사용 금지.\n"
    "4. 입력의 idx 를 결과에 그대로 보존하여 순서 매칭.\n"
    "5. JSON 형식만 응답: {\"results\": [{\"idx\": 0, \"friendly_title\": \"...\", \"friendly_summary\": \"...\"}, ...]}"
)


def batch_generate_friendly(items: list, texts: list) -> dict:
    """N건 공고를 한 번의 GPT 호출로 통역.

    items[i]: {id, title, organization, ...}
    texts[i]: 본문 (각 최대 3000자, 호출자가 잘라 전달)
    Returns: {idx: {friendly_title, friendly_summary}, ...} — 성공한 항목만.
    실패 시 빈 dict (호출자가 단건 fallback 결정).
    """
    api_key = os.environ.get("OPENAI_API_KEY", "")
    ...
    try:
        ...
        print(f"[ai_translate.batch] GPT 호출 실패 (n={len(items)}): {e}")
        return {}
    ...
    for r in results:
        ...
        if not (0 <= idx < len(items)):
            continue
        ft = str(r.get("friendly_title") or "").strip()
        fs = str(r.get("friendly_summary") or "").strip()
        if not ft and not fs:
            continue
        out[idx] = {"friendly_title": ft, "friendly_summary": fs}
    return out
